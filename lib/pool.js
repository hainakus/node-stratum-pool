...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    function SetupJobManager(){
        ...
        _this.jobManager.on('newBlock', function(blockTemplate){
            ...
            if (_this.stratumServer) {
                _this.stratumServer.broadcastMiningJobs(blockTemplate.getJobParams(options.coin.algorithm === "equihash"));
            }
        }).on('updatedBlock', function(blockTemplate){
            //Check if stratumServer has been initialized yet
            if (_this.stratumServer) {
                var job = blockTemplate.getJobParams(options.coin.algorithm === "equihash");
                job[8] = false;
    ...
    function StartStratumServer(finishedCallback){
        ...
        _this.stratumServer.on('started', function(){
            ...
            _this.stratumServer.broadcastMiningJobs(_this.jobManager.currentJob.getJobParams(options.coin.algorithm === "equihash"));
            finishedCallback();
            ...
            client.on('difficultyChanged', function(diff){
                ...
                this.sendMiningJob(_this.jobManager.currentJob.getJobParams(options.coin.algorithm === "equihash"));
                
            }).on('submit', function(params, resultCallback){
                var solution = (options.coin.algorithm === "equihash") ? params.solution : null;
    ...
    this.setVarDiff = function(port, varDiffConfig) {
        ...
        _this.varDiff[port].on('newDifficulty', function(client, newDiff) {
            ...
            /*if (options.varDiff.mode === 'fast'){
                ...
                var job = _this.jobManager.currentJob.getJobParams(options.coin.algorithm === "equihash");
                job[8] = false;
