...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    function OnBlockchainSynced(syncedCallback){
        ...
        var checkSynced = function(displayNotSynced){
            if(options.coin.isMultiAlgo)
            {
                _this.daemon.cmd('getblocktemplate', [{"capabilities": [ "coinbasetxn", "workid", "coinbase/append" ], "rules": [ "segwit" ]}, options.coin.MultiAlgoString], function(results){
    ...
    function SubmitBlock(blockHex, callback){
        ...
        var input = blockHex.toString()
        var data = Buffer.from(input, 'hex')
        rpcCommand = 'submitminingsolution';
        
        rpcArgs = [];
        console.log('SJOOOOOONNN' ,input.toString() )
    ...
    function SetupJobManager(){
        ...
        _this.jobManager.on('newBlock', function(blockTemplate){
            ...
            console.log('hexxxxxxxxxxxxxx', Buffer.from(blockHex, 'hex').toString('hex'));
    ...
    function GetBlockTemplate(callback){
        if(options.coin.isMultiAlgo)
        {
            _this.daemon.cmd('getblocktemplate',
                [{"capabilities": [ "coinbasetxn", "workid", "coinbase/append" ], "rules": [ "segwit" ]}, options.coin.MultiAlgoString],
                function(result){
                    if (result.error){
