...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    function OnBlockchainSynced(syncedCallback){
        ...
        var checkSynced = function(displayNotSynced){
                ...
                _this.daemon.cmd('getminingcandidate', [], function(results){ 
                    console.log(results)
                    store('candidatejobId', results[0].response.id);
                });
        };
     ...
     */
    async function SubmitBlock(blockHex, callback){
        blockHex1 =  await ns.get('blockHex');
        //console.log(JSON.parse(blockHex1));
        var rpcCommand, rpcArgs;
            //blockHex = hex2a(blockHex)
       
       
        rpcCommand = 'submitminingsolution';
        
        rpcArgs = [];
       
         
       
            console.log('BBBBBBBBBBB', blockHex1);
            ...
            _this.daemon.cmd(rpcCommand,
               ...
               function(results){
                   ...
                   emitLog('Submitted Block using ' + JSON.stringify(results) + ' successfully to daemon instance(s)');
                   callback();
               }
           );
    ...
    function SetupJobManager(){
        ...
        _this.jobManager.on('newBlock', function(blockTemplate){
            ...
            console.log('hexxxxxxxxxxxxxx',blockHex? blockHex.toString(): '');
            console.log(shareData);
           
            var isValidShare = !shareData.error;
            var isValidBlock = !!blockHex;
            console.log('val',isValidBlock)
    ...
    function DetectCoinData(finishedCallback){
        ...
        _this.daemon.batchCmd(batchRpcCalls, function(error, results){
            ...
            for (var i = 0; i < results.length; i++){
                ...
                if (rpcCall !== 'submitminingsolution' && (r.error || !r.result)){
            ...
            if (rpcResults.submitminingsolution.message === 'Method not found'){
                options.hasSubmitMethod = false;
            }
            else if (rpcResults.submitminingsolution.code === -1){
    ...
    function GetBlockTemplate(callback){
        _this.daemon.cmd('getminingcandidate', [], function(results){ 
            console.log(results)
            _this.id = results[0].response.id;
          
                store('miningcandidate', _this.id);
                store('candidatejobId', results[0].response.id);
            ...
            _this.daemon.cmd('getblocktemplate',
                ...
                function(result){
                    if (result.error){
                        ...
                        console.log('JOBBBBBBBBB', result.response)
