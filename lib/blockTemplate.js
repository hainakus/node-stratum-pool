...
var BlockTemplate = module.exports = function BlockTemplate(jobId, rpcData, poolAddressScript, extraNoncePlaceholder, reward, txMessages, recipients){
    ...
    // Equihash block header https://github.com/zcash/zips/blob/master/protocol/protocol.pdf
    
    this.serializeEquihashHeader = function(merkleRoot, nTime, nonce){
    ...
    this.serializeEquihashBlock = function(header, coinbase, solution){
        return Buffer.concat([
            header,
            solution,
    ...
    this.getJobParams = function(equihashblock){
        if (!this.jobParams){
            if(equihashblock)
            {
                this.jobParams = [
