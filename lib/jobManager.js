...
var JobManager = module.exports = function JobManager(options){
    ...
    var shareMultiplier = Number(Math.pow(10, 9)).toFixed(12);
    ...
    this.processShare = function(jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName){
        ...
        var headerBigNum = bignum.fromBuffer(headerHash, {endian: 'big', size: 0});
        ...
        var shareDiff =  job.difficulty / (Number(
        ...
        * shareMultiplier
        ...
        console.log('ERRRRR',job.rpcData)
        var blockDiffAdjusted = job.difficulty * 1;

        //Check if share is a block candidate (matched network difficulty)
        console.log(job.difficulty)
      //  console.log(job.target.ge(headerBigNum))
      
        var jobtarget = Buffer(job.rpcData.bits, 'hex').readFloatBE(0)
        console.log('target', jobtarget)
        console.log('header', headerBigNum.toNumber())
          console.log('job', bignum(jobtarget).ge(headerBigNum))
        if (bignum(jobtarget).ge(headerBigNum)){
            ...
            if (false)
                ...
                console.log('diff',shareDiff/ difficulty)
