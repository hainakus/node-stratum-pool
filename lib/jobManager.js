...
var JobManager = module.exports = function JobManager(options){
    ...
    this.processShare = function(jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, ehsolution){
        ...
        var headerBuffer;
        var headerHash;
        var headerSolutionBuffer;
        
        if(options.coin.algorithm === "equihash")
        {
            headerBuffer = job.serializeEquihashHeader(merkleRoot, nTime, nonce);
            headerSolutionBuffer = new Buffer.concat([headerBuffer, new Buffer(ehsolution, 'hex')]);
            headerHash = util.sha256d(headerSolutionBuffer);
        }
        else
        {
        ...
        if(options.coin.algorithm === "equihash")
        {
            ...
            if (hashDigest(headerBuffer, new Buffer(ehsolution.slice(6), 'hex')) !== true) {
        ...
        if (job.target.ge(headerBigNum)){
            if(options.coin.algorithm === "equihash")
            {
                blockHex = job.serializeEquihashBlock(headerBuffer, coinbaseBuffer, new Buffer(ehsolution, 'hex')).toString('hex');
            ...
            {
                if(options.coin.algorithm === "equihash")
                    blockHashInvalid = util.reverseBuffer(util.sha256d(headerSolutionBuffer)).toString('hex');
                else
