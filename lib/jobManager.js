...
var JobManager = module.exports = function JobManager(options){
    ...
    var shareMultiplier = Number(Math.pow(10, 6)).toFixed(12);
    ...
    this.processShare = function(jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName){
      ...
      //  console.log(job.target.ge(headerBigNum))
        ...
        var jobtarget = Buffer(job.rpcData.bits, 'hex')
        var jobTarget = bignum.fromBuffer(jobtarget, {endian: 'big', size: 0});
        console.log('target', jobTarget.toNumber())
        console.log('header', shareDiffCheck)
          console.log('job', jobTarget.ge(shareDiffCheck))
        if (shareDiff.toFixed(8) >= job.difficulty){
            blockHex = job.serializeBlock(headerBuffer, coinbaseBuffer, nonce).toString('hex');
        ...
        _this.emit('share', {
            ...
            difficulty: difficulty,
            shareDiff: Number(shareDiff.toFixed(8)),
