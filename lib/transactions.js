...
var generateOutputTransactions = function(poolRecipient, recipients, rpcData){
    ...
    if (rpcData.masternode_payments_started && rpcData.masternode.payee) {
        var payeeReward = rpcData.masternode.amount;

        reward -= payeeReward;
        rewardToPool -= payeeReward;

        var payeeScript = new Buffer(rpcData.masternode.script, 'hex');
    ...
    if (rpcData.treasury !== undefined && rpcData.treasury !== null){
        
        reward -= rpcData.treasury.amount;
        rewardToPool -= rpcData.treasury.amount;
        
        txOutputBuffers.push(Buffer.concat([
            new Buffer(rpcData.treasury.hex, 'hex')
        ]));
    }
